...
typedef struct {
    int ****voxel_atoms; // 3D array, each element is an integer array, storing the atomic index in that voxel
    int ****voxel_count; // 3D array, each element is an integer pointer pointing to the atomic count of that voxel
    float min_x, min_y, min_z; // Minimum coordinates of the grid (lower limit of spatial range)
    float voxel_size;          // The size of voxels (edge length, unit?)
    int num_x, num_y, num_z;   // The number of voxels in the x, y, and z directions of the grid
} AtomGrid;
...
void init_atom_grid(AtomGrid *grid, float min_x, float min_y, float min_z, float max_x, float max_y, float max_z, float voxel_size);
void add_atom_to_grid(AtomGrid *grid, int atom_index, float x, float y, float z);
void get_nearby_atoms(AtomGrid *grid, float x, float y, float z, int *atom_indices, int *num_nearby);
int cal_dis_nearby_optimized(struct atoms *atom_data, int *atom_indices, int num_nearby, 
                            float *r_sum, float *r_sum_sq, 
                            float x, float y, float z);
void free_atom_grid(AtomGrid *grid) ;
...
// generation conforatiom
	...
	if(c_genf!=0){
		...
		if(genc_method==1) printf("You chooed method is: [1] Monte Carlo method \n");	
			...
			printf("You chooed method is: [%d], undefined exit\n",genc_method);
			return -1;
		}		
	}
// Energy minimization method 0 MMF94 1 MOPAC 3 MMF94+MOPAC
	if(c_method_mm!=0){
		...
		if((mm_method==2)||(mm_method==3)){
			if(mm_method==2)
			printf("You chooed method for generation conformation is: [%d]:MOPAC \n",mm_method);
			if(mm_method==3)
			printf("You chooed method for generation conformation is: [%d]:MMF94+MOPAC \n",mm_method);
			...
			if(ifpath>=1)
				printf("You entered path of mopac is : %s \n",mopacphath);
			else{
				printf("You pressd the Enter key or  entered path of mopac is: %s is wrong use [\\MOPAC\\MOPAC2016.exe]\n",mopacphath);	
				#ifdef _WIN32
    			// Windows  
			...
			printf("You chooed method for generation conformation is: [%d]:MMF94 \n",mm_method);
		}
		else{
			printf("You chooed method for generation conformation is: [%d], undefined exit\n",mm_method);
			return -1;
		}
	}
/**
...
void init_atom_grid(AtomGrid *grid, float min_x, float min_y, float min_z, float max_x, float max_y, float max_z, float voxel_size) {
    ...
    grid->num_x = (int)((max_x - min_x) / voxel_size) + 1;
    grid->num_y = (int)((max_y - min_y) / voxel_size) + 1;
    grid->num_z = (int)((max_z - min_z) / voxel_size) + 1;

    // Allocate memory for voxel atoms and voxel count
    grid->voxel_atoms = (int ****)malloc(grid->num_x * sizeof(int ***));
    grid->voxel_count = (int ****)malloc(grid->num_x * sizeof(int ***));
    int i;
    for ( i = 0; i < grid->num_x; i++) {
        grid->voxel_atoms[i] = (int ***)malloc(grid->num_y * sizeof(int **));
        grid->voxel_count[i] = (int ***)malloc(grid->num_y * sizeof(int **));
        int j;
        for ( j = 0; j < grid->num_y; j++) {
            grid->voxel_atoms[i][j] = (int **)malloc(grid->num_z * sizeof(int *));
            grid->voxel_count[i][j] = (int **)malloc(grid->num_z * sizeof(int *));
            int k;
            for ( k = 0; k < grid->num_z; k++) {
                // Initialize each voxel: the initial capacity of the atomic array is 10, and the initial atomic count is 0
                grid->voxel_atoms[i][j][k] = (int *)malloc(10 * sizeof(int));
                grid->voxel_count[i][j][k] = (int *)malloc(sizeof(int));
                *(grid->voxel_count[i][j][k]) = 0;
            }
        }
    }
}
...
void add_atom_to_grid(AtomGrid *grid, int atom_index, float x, float y, float z) {
    // Calculate the voxel index where the atom is located
    int i = (int)((x - grid->min_x) / grid->voxel_size);
    int j = (int)((y - grid->min_y) / grid->voxel_size);
    int k = (int)((z - grid->min_z) / grid->voxel_size);
    // Check if the index is within the grid range
    if (i < 0 || i >= grid->num_x || j < 0 || j >= grid->num_y || k < 0 || k >= grid->num_z) {
        return; // If it exceeds the range, skip the atom
    }
    int count = *(grid->voxel_count[i][j][k]); // Obtain the number of atoms in the current voxel
    // If the current quantity reaches the capacity, reallocate the memory (doubling the capacity)
    if (count >= 10) {
        grid->voxel_atoms[i][j][k] = (int *)realloc(grid->voxel_atoms[i][j][k], (count * 2) * sizeof(int));
    }
    // Add atomic index to voxel array and update count
    grid->voxel_atoms[i][j][k][count] = atom_index;
    *(grid->voxel_count[i][j][k]) = count + 1;
}
...
void get_nearby_atoms(AtomGrid *grid, float x, float y, float z, int *atom_indices, int *num_nearby) {
    // Calculate the voxel index where the point is located
    int i = (int)((x - grid->min_x) / grid->voxel_size);
    int j = (int)((y - grid->min_y) / grid->voxel_size);
    int k = (int)((z - grid->min_z) / grid->voxel_size);
    *num_nearby = 0; //Initialize the number of nearby atoms to 0 
    // Traverse the voxel and its 26 adjacent voxels (out of a total of 27) 
    int di;
    for ( di = -1; di <= 1; di++) {
    	int dj;
        for (dj = -1; dj <= 1; dj++) {
        	int dk;
            for ( dk = -1; dk <= 1; dk++) {
                int ni = i + di, nj = j + dj, nk = k + dk; // Adjacent voxel index
                // Check if adjacent voxels are within the grid range 
                if (ni >= 0 && ni < grid->num_x && nj >= 0 && nj < grid->num_y && nk >= 0 && nk < grid->num_z) {
                    int count = *(grid->voxel_count[ni][nj][nk]); // The number of atoms in this voxel 
                    //  Add all atomic indices in the voxel to the output array
                    int c;
                    for ( c = 0; c < count; c++) {
                        atom_indices[(*num_nearby)++] = grid->voxel_atoms[ni][nj][nk][c];
                    }
                }
            }
        }
...
int cal_dis_nearby_optimized(struct atoms *atom_data, int *atom_indices, int num_nearby, 
    ...
    min_dis_sq = (atom_data[atom_indices[0]].xa - x) * (atom_data[atom_indices[0]].xa - x) +
                 (atom_data[atom_indices[0]].ya - y) * (atom_data[atom_indices[0]].ya - y) +
                 (atom_data[atom_indices[0]].za - z) * (atom_data[atom_indices[0]].za - z);
    index_min = atom_indices[0];
    
    for (i = 0; i < num_nearby; i++) {
        int idx = atom_indices[i];
        float dx = atom_data[idx].xa - x;
        float dy = atom_data[idx].ya - y;
        float dz = atom_data[idx].za - z;
        distance_sq = dx*dx + dy*dy + dz*dz;
        
        if (min_dis_sq > distance_sq) {
            min_dis_sq = distance_sq;
            index_min = idx;
        }
        
      
        if (distance_sq <= r_sum_sq[idx]) {
            fit++;
        }
    }
    
  
    float min_dis = sqrt(min_dis_sq);
    if (fit == 0) {
        
        float threshold = r_sum[index_min] + 2.5;
...
void free_atom_grid(AtomGrid *grid) {
    ...
    for ( i = 0; i < grid->num_x; i++) {
        ...
        for ( j = 0; j < grid->num_y; j++) {
        	int k;
            for ( k = 0; k < grid->num_z; k++) {
                free(grid->voxel_atoms[i][j][k]);
                free(grid->voxel_count[i][j][k]);
            }
            free(grid->voxel_atoms[i][j]);
            free(grid->voxel_count[i][j]);
        }
        free(grid->voxel_atoms[i]);
        free(grid->voxel_count[i]);
    }
    free(grid->voxel_atoms);
    free(grid->voxel_count);
}
...
// setting grid_gap
    ...
    if(box_vol < 200.0) grid_gap = 0.21;
    else if (box_vol >= 200.0 && box_vol < 500.0) grid_gap = 0.233;	
    else if (box_vol >= 500.0 && box_vol < 1000.0) grid_gap = 0.228;	
    else grid_gap = 0.235;
#elif defined(GAS)
	box_vol=(minmax.xmax-minmax.xmin)*(minmax.ymax-minmax.ymin)*(minmax.zmax-minmax.zmin);
	...
	float *r_sum = (float *)malloc(atom_number * sizeof(float));
	float *r_sum_sq = (float *)malloc(atom_number * sizeof(float));
	int a;
    for ( a = 0; a < atom_number; a++) {
        r_sum[a] = atom_data[a].rvdw + water_radius;
        r_sum_sq[a] = r_sum[a] * r_sum[a];
    }

    // ========== Start adding atomic grid index code==========
    // Before calculating grid points, build an atomic grid index first
        
    float max_atom_radius = 0.0;
 
    for ( a = 0; a < atom_number; a++) {
        if (atom_data[a].rvdw > max_atom_radius) {
            max_atom_radius = atom_data[a].rvdw;
        }
    }
    float atom_voxel_size = max_atom_radius+water_radius;
    
    
    AtomGrid atom_grid;
    init_atom_grid(&atom_grid, minmax.xmin - 4.0, minmax.ymin - 4.0, minmax.zmin - 4.0,
                   minmax.xmax + 4.0, minmax.ymax + 4.0, minmax.zmax + 4.0, atom_voxel_size);
    
    // Add all atoms to the grid
   
    for ( a = 0; a < atom_number; a++) {
        add_atom_to_grid(&atom_grid, a, atom_data[a].xa, atom_data[a].ya, atom_data[a].za);
    }
    
    // Allocate an array to store nearby atomic indices (assuming a maximum of 1000 nearby atoms)
    int *nearby_atoms = (int *)malloc(1000 * sizeof(int));
    int num_nearby;
    // ========== End of adding atomic grid index code ==========
    ...
    // ==========Starting the cycle of modifying the grid point state calculation==========
    // 
    for(i=0;i<=count-1;i++)
    {
        // Retrieve atoms near the current grid point
        get_nearby_atoms(&atom_grid, grid_data[i].x, grid_data[i].y, grid_data[i].z, nearby_atoms, &num_nearby);
        
        // Calculate states using nearby atoms instead of all atoms
        grid_data[i].state = cal_dis_nearby_optimized(atom_data, nearby_atoms, num_nearby, 
                                                     r_sum, r_sum_sq, 
                                                     grid_data[i].x, grid_data[i].y, grid_data[i].z);
        
     //   grid_data[i].state = cal_dis_nearby(atom_data, nearby_atoms, num_nearby, water_radius, grid_data[i].x, grid_data[i].y, grid_data[i].z);
        
        if(grid_data[i].state==-1){
            printf("\nError unsupport atom type.\n");
            
            // ========== Start adding memory release code ==========
            // Release atomic grid index memory before returning
            free(nearby_atoms);
            free_atom_grid(&atom_grid);
            // ==========End of adding memory release code ==========
    ...
    free(r_sum);
    free(r_sum_sq);
    // ========== End of cycle for modifying grid point state calculation ==========
...
#ifdef SOLID
    suface = 1.144 * suface + 9.915;
#elif defined(LIQUID)
    suface = 1.193 * suface + 8.602;
#elif defined(GAS)
...
{
	printf("error: cant not open file suf_data.XYZ\n");
	return -1;
}
...
float get_dis()
{
	...
	printf("You selected Boltzmann distribution isï¼š%f%%\n",bloz);
	return bloz;	
}
void add_prefix(char *newnamef, char *name, char *add)
{
