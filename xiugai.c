...
//float cal_vol(struct atom_data *data_arry,float xmin,float xmax,float ymin,float ymax,float zmin,float zmax, int arry_count, int number_count);
...
int main(int argc, char *argv[])
{
    ...
    char mopacphath[100];  // 设置默认输出文件
    int opt,if_trace_file=0,genc_method=0,mm_method=0,mopac_multi=1,mopac_charge=0,out_data=0;  // 显式声明opt变量
...
// -m <number> : Energy minimization method 0 MMF94 1 MOPAC 3 MMF94+MOPAC
    ...
    while ((opt = getopt(argc, argv, "i:s::dgm")) != -1) {  // 修正选项字符串
        switch (opt) {
            case 'i':
                input_file = optarg;
                break;
            case 'd':
                out_data=1;  // 正确处理带参数的-d选项
                break;
        case 's': {
            // 处理所有可能的参数格式
            char *arg = optarg;
            if (arg == NULL) {
                // 尝试检查下一个参数是否为数字（支持 -s 1.5）
            ...
            if (endptr == arg || *endptr != '\0') {
                fprintf(stderr, "Error: -s 参数必须是浮点数\n");
                return 1;
            }
            water_flag = s_value;
            break;
        	...
        	case 'm':
            // -m 不带参数（纯标志）
            	c_method_mm= 1; // 假设用全局变量标记
            break;
            default:
                fprintf(stderr, "用法: %s -i 输入文件 [-d 输出文件] [-s SASA值] [-r 水分子半径]\n", argv[0]);
                return 1;
        }
    }
...
// Energy minimization method 0 MMF94 1 MOPAC 3 MMF94+MOPAC
	if(c_method_mm!=0){
		...
		if((mm_method==2)||(mm_method==3)){
			...
			printf("vacuum ε=0.0 water ε=78.3553, N-Octanol ε=9.8629, DMSO：ε=46.826, Heptane: ε=1.9113 ...\n");	
			fgets(buffer_command, sizeof(buffer_command), stdin);
...
/**************************************/
	...
	if((c_genf==0)&&(c_method_mm==0)){
		...
		if(mm_method==3){
			cal_obabel_mmf(input_file);	//obable
			add_prefix(newname, input_file, "mm_");
			if_blot=ask_if_bloz();
			if(if_blot==0){
				cal_mopac_mmf(newname,mopac_multi,mopac_charge, dielectric,mopacphath ); //mopaca	
				if_blot=ask_if_bloz();
				if(if_blot==0){
					cal_type=10;//##########################
				}
		...
		if(mm_method==3){
			cal_obabel_mmf(newname);	//obable
			add_prefix(newname1, newname, "mm_");
			if_blot=ask_if_bloz();
			if(if_blot==0){
				cal_mopac_mmf(newname1,mopac_multi,mopac_charge, dielectric,mopacphath ); //mopaca	
				if_blot=ask_if_bloz();
				if(if_blot==0){
					cal_type=18;//##########################
				}
...
if((if_trace_file)>0){
	...
	//	printf("%d %f %f %f ----(%f %f %f) \n",i,frame_result[i].surf,frame_result[i].psa,frame_result[i].vol,asuf,apsa,avol);
	}
...
struct min_max  get_min_max(struct atoms *atom_data,int atom_number)
{
	...
	for(i=0;i<=atom_number-1;i++)
	{
		//if(atom_data[i].xa<=xmin) xmin=atom_data[i].xa;
		xmin= (atom_data[i].xa < xmin) ? atom_data[i].xa : xmin;
		//if(atom_data[i].xa>=xmax) xmax=atom_data[i].xa;
		xmax= (atom_data[i].xa > xmax) ? atom_data[i].xa : xmax;
		
		
		//if(atom_data[i].ya<=ymin) ymin=atom_data[i].ya;
		//if(atom_data[i].ya>=ymax) ymax=atom_data[i].ya;
		ymin= (atom_data[i].ya < ymin) ? atom_data[i].ya : ymin;
		ymax= (atom_data[i].ya > ymax) ? atom_data[i].ya : ymax;
		
		//if(atom_data[i].za<=zmin) zmin=atom_data[i].za;
		//if(atom_data[i].za>=zmax) zmax=atom_data[i].za;
...
//	printf(" %d  ",atom_data[minatom].ele);
...
//	else if(atom_data[minatom].ele==15) return 1;
	else return 0;
...
//	*xyz_file_number=0;
	*xyz_file_number=1;
	...
	}	else if (file_type( file_name )==4)
	{
		...
		//printf("line_count is %d atom_number is %d\n",line_count,atom_number);
		file_number=line_count/(atom_number+2);
		//printf("file number is %d",file_number);
		*p_atom_number=atom_number;
	...
	//	printf("\n %d %d  count %d\n",atom_number,file_number,line_count);
...
//det_suf=0.3044*suface + 5.3462;
//suface=suface+det_suf;
//det_suf=0.295*suface + 1.6153;
//suface=suface+det_suf;
...
FILE *outsuf_fp;
if((outsuf_fp=fopen(out_suf,"w"))==NULL) 
{
	printf("error: cant not open file [%s]\n",outsuf_fp);
	return -1;
}
for(i=0;i<=box_x*box_y*box_z-1;i++)
{
	
	if  (grid_data[i].state==1)
	{    
		if(outsuf_fp!=NULL)
			fprintf(outsuf_fp,"ATOM      1  DOT DOT     1    %8.3f%8.3f%8.3f  1.00  0.00          Ve\n",grid_data[i].x,grid_data[i].y,grid_data[i].z);		
	}
}
...
//	if ((grid_data[i].state==2) ||(grid_data[i].state==1) || (grid_data[i].state==200)) 
...
int is_obabel_installed() {
    int ret;
#ifdef _WIN32
    // Windows 使用 where 命令查找 obabel.exe
    ret = system("where obabel.exe > nul 2>&1");
#else
    // Unix/Linux/macOS 使用 which 命令查找 obabel
...
/***************************		
		for(i=1;i<=file_number;i++){
			...
			if(temp==NULL){
				printf("Open file fialed temp.xyz");
				fclose(temp);
				fclose(mmresult);
				free(atom_info_data);
				free(atom_data);
			...
			fprintf(temp,"%d\n",atom_number);
  			fprintf(temp,"%d\n",i);		
			for(j=0;j<=atom_number-1;j++){
				switch(atom_data[(i-1)*atom_number+j].ele){
					case 1 : fprintf(temp,"%3s","H") ; break;
					case 5 : fprintf(temp,"%3s","B") ; break;
					case 6 : fprintf(temp,"%3s","C") ; break;
					case 7 : fprintf(temp,"%3s","N") ; break;
					case 8 : fprintf(temp,"%3s","O") ; break;
					case 9 : fprintf(temp,"%3s","F") ; break;
					case 14 : fprintf(temp,"%3s","Si") ; break;
					case 15 : fprintf(temp,"%3s","P") ; break;
					case 16 : fprintf(temp,"%3s","S") ; break;
					case 17 : fprintf(temp,"%3s","Cl") ; break;
					case 35 : fprintf(temp,"%3s","Br") ; break;
					case 53 : fprintf(temp,"%3s","I") ; break;
				}
				fprintf(temp,"%15.5f%15.5f%15.5f\n",atom_data[(i-1)*atom_number+j].xa,atom_data[(i-1)*atom_number+j].ya,atom_data[(i-1)*atom_number+j].za);
			}
			fclose(temp);
			command[0]='\0';
			strcat(command,"Obabel ");
			...
			strcat(command," --minimize  --append \"Energy\"  --ff MMFF94   --steps 3000 ");
			printf("Frame %d of %d ",i,file_number);
			printf("%s",command);
			system(command);
			FILE *mm_temp;
			char fstring[300];
			...
			if(mm_temp!=NULL){
				...
				printf("Open file fialed mm_temp.xyz");
				fclose(temp);
				fclose(mm_temp);
				fclose(mmresult);
				free(atom_info_data);
